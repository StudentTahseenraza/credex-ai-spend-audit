...
export function SpendHealthScore({ tools, className }: SpendHealthScoreProps) {
  ...
  const calculatePlanEfficiency = useCallback((): number => {
    ...
    tools.forEach(tool => {
      ...
      const efficiency = Math.max(
        0,
        100 - (savings / (tool.monthlySpend || 1)) * 100
      );
  ...
  const calculateToolRedundancy = useCallback((): number => {
    ...
    const categories = tools.map(
      t => toolCategories[t.name] || 'other'
    );
  ...
  const calculateSeatUtilization = useCallback((): number => {
    ...
    const avgSeats =
      tools.reduce((sum, t) => sum + t.seats, 0) / tools.length;
  ...
  const calculateHealthScore = useCallback(() => {
    ...
    const metricsList: HealthMetric[] = [
      {
        name: 'Plan Efficiency',
        score: Math.round(planEfficiency),
        weight: 0.25,
        description:
          'How well your current plans match actual usage patterns',
      },
      {
        name: 'Tool Redundancy',
        score: Math.round(toolRedundancy),
        weight: 0.15,
        description:
          'Overlap between similar AI tools in your stack',
      },
      {
        name: 'Seat Utilization',
        score: Math.round(seatUtilization),
        weight: 0.2,
        description:
          'Percentage of paid seats actively generating value',
      },
      {
        name: 'Spend Efficiency',
        score: Math.round(spendEfficiency),
        weight: 0.25,
        description:
          'Cost per user compared to industry benchmarks',
      },
      {
        name: 'Workflow Fit',
        score: Math.round(workflowFit),
        weight: 0.15,
        description:
          'Alignment between tools and your primary use case',
      },
    ];
    ...
    calculateToolRedundancy,
    calculateSeatUtilization,
    calculateSpendEfficiency,
    calculateWorkflowFit,
  ]);

  const { totalScore: score, metrics } = useMemo(() => {
  ...
  return (
    <div
      className={cn(
        'bg-gradient-to-br from-white to-gray-50 rounded-2xl border border-gray-200 p-6 shadow-sm',
        className
      )}
    >
        ...
        <div className="relative group">
          ...
          <div className="absolute bottom-full right-0 mb-2 hidden group-hover:block z-50">
            <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 max-w-xs">
              <p>
                Combined score based on plan efficiency,
                tool redundancy, seat utilization, spend
                efficiency, and workflow alignment
              </p>
            </div>
          </div>
        </div>
